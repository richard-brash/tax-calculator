...
var _MakePDF = function(){
    ...
    this.makeIt = function(data, callback){
        ...
        logger.errorLog.info('We finished defining dd (Document Definition...');

        var pdfDoc = printer.createPdfKitDocument(dd);

        logger.errorLog.info('We created the pdfDof from the document definition -- createPdfKitDocument(dd);');

        var chunks = [];
        var result;
      
        pdfDoc.on('data', function (chunk) {
            logger.errorLog.info('pushing chunks...');
            chunks.push(chunk);
        });

        pdfDoc.on('end', function () {
            logger.errorLog.info('on end...  concatnating...');      
            result = Buffer.concat(chunks);
            logger.errorLog.info('we have the results.');      
            logger.errorLog.info(result.toString('base64'));      
